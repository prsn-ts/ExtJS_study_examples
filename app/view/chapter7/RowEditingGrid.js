Ext.define('MyExtJS.view.chapter7.RowEditingGrid', {
...
//            },
    ...
    infoEdit : function(event, editor, context, e){
        ...
        console.log(event , ':', record, field, value, rowIdx, colIdx, grid, row, column, editor);

        console.log(event);
        console.log(record); //변경되지 않은 데이터 모델 레코드
        console.log(field); //편집된(퍈집되고 있는) 필드
        console.log(value); //선택한 곳의 dataIndex 이름에 부합하는 값
        console.log(rowIdx); //편집 대상 그리드 행의 인덱스
        console.log(colIdx); //편집 대상 그리드 열의 인덱스
        console.log(grid); //그리드 객체
        console.log(row); //편집 대상 그리드 행
        console.log(column); //편집 대상 그리드 열
        console.log(editor); //편집이 발생한 에디터(roweditor 또는 celleditor)
